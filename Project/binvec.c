...
void main(void)
{
  ...
  while (cont == 1)
  { /* main loop, vectors are generated in lexicographical order */
    numb++;
    ...
    if (top > 0)
    {               /* if current vector is not lexicographic max */
      vec[top] = 1; /* set vec[top] = 1 and */
      if (top < n)
      { /* set vec[i]=0 for all indices i larger than top */
        for (i = top + 1; i <= n; i++)
        {
          vec[i] = 0;
        }
    ...
    {
      cont = 0;
    } /* current vector is lexicographic max so stop */
  }
