...
void bfs(int *Q,int *d,int *memory,cell *List,int *freeL,int s,int *maxd,int *maxb); //幅優先探索
void addqueue(int *Q,int *m,cell* L,int *f,int *d,int *maxd,int *maxb,int p,int v); //キューへの追加や深さの更新を行う関数
void enqueue(int *Q, int a); //キューに追加する
int dequeue(int *Q); //キューから取り除く
void symmetrymemory(int *memory,cell *L,int *f, int a); //反転回転を考慮してハッシュに保存する関数
int allocate_object(cell *L, int *f); //リストから確保する
void chained_hash_insert(int *A, cell *L, int x); //ハッシュへ挿入する関数
int chained_hash_search(int *A, cell *L, int p); //ハッシュから探索する関数
int hash_val(int a); 
void show(int board[][16], int *ans); //結果を表示する関数
...
int main(void) {
    ...
    int ans[16];
    for(i=0; i<16; i++) ans[i]=-1;
    int board[16][16]; //盤面を保存するための配列
    for(i=0; i<16; i++) {
        board[i][0]=-1;
    }
    
    for(i=0;i<16;i++) { 
        if(ans[i]!=-1) continue; //盤面が計算済みならスキップ
        int memory[MOD]; /* 盤面の一つ前の手を保存する配列 ハッシュにするべきなやつ */
        for(j=0;j<MOD;j++) memory[j]=-1;
        cell List[10000]; //余裕をもって10000にする
        for(j=0;j<10000;j++) List[j].next=j+1;
        List[9999].next=-1;
        int freeL=0;
        ...
        int a,b;
        a=i;
        b=i/4*4+(3-i%4); //初期盤面の反転
        ans[b]=15-maxd;
        for(j=0;j<3;j++) {
            a=(80-4*a)%17; //駒がないマスが1つしかないので回転がこれでできる
            b=(80-4*b)%17;
            ans[a]=15-maxd;
            ans[b]=15-maxd;
        }
...
void bfs(int *Q,int *d,int *m,cell *L,int *f,int s,int *maxd,int *maxb){
    ...
    while(head!=tail) {
        v=dequeue(Q);
        for(i=4;i<=11;i++) { //縦向きの駒取りを探索
            if((v>>i)&1) {
                if(((v>>(i-4))&1)^((v>>(i+4))&1)) { //こまを取れるか確認して取れるならbitを反転させて保存する。
                    int p=v;
                    p^=(273<<(i-4));
...
void addqueue(int *Q,int *m,cell* L,int *f,int *d,int *maxd,int *maxb,int p,int v){
    ...
    enqueue(Q,p);
    if(*maxd<d[p]) { //もし来たことのない深さなら深さと盤面を更新する
        *maxd=d[p];
        *maxb=p;
    }
...
void symmetrymemory(int *memory,cell *L,int *f,int a) { //反転と回転をメモリ―に入れる
    int i,j,b=0;
    for(i=0;i<4;i++) {
        for(j=0;j<4;j++) {
...
void show(int board[][16], int *ans) {
    ...
    printf("最小の駒数になるとり方を表示します\n");//3通りのみ表示し、そのほかは回転や反転で対応しているので表示しない。
    for(i=0;i<16;i++) {
        if(board[i][0]==-1) continue; //ボードに記録した盤面だけ出力する
        B=A^(1<<i);
...
int allocate_object(cell *L, int *f) { /* リスト配列と空きアドレスリスト先頭はポインタ渡し */
    int x;
